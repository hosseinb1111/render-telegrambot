...
function configWarnings(){const missing=[];if(!BOT_TOKEN)missing.push("BOT_TOKEN");if(!OR_KEY)missing.push("OPENROUTER_API_KEY");if(!WEBHOOK_SECRET)missing.push("WEBHOOK_SECRET");if(!WEBHOOK_PATH_TOKEN)missing.push("WEBHOOK_PATH_TOKEN");if(missing.length)console.error(`Missing required environment variables: ${missing.join(", ")}`)}configWarnings();
...
function chooseReaction(text,image=false,userId=""){const s=String(text||"").trim(),score=new Map(RX.map(e=>[e,0])),add=(e,p)=>score.set(e,(score.get(e)||0)+p);if(image)add("👀",8);if(/😭|😢|sad|depressed|غمگین|ناراحت|دلتنگ|ناامید/i.test(s)){add("😢",30);add("😭",10)}if(/😡|🤬|angry|furious|عصبانی|مزخرف|افتضاح/i.test(s))add("😡",30);if(/😂|🤣|haha+|lol|lmao/i.test(s))add("😂",30);if(/❤️|😍|🥰|love|عشق|دوست دارم/i.test(s))add("❤️",28);if(/🔥|excited|let's go|lets go|بزن بریم/i.test(s))add("🔥",25);if(/🤯|😮|😲|no way|جدی؟|واقعا؟/i.test(s))add("😮",26);if(RX_PATTERNS.praise.test(s))add("👏",8);if(RX_PATTERNS.help.test(s))add("🤔",6);if(RX_PATTERNS.code.test(s))add("🧠",7);if(RX_PATTERNS.science.test(s))add("🧠",7);if(RX_PATTERNS.money.test(s))add("🧐",7);if(RX_PATTERNS.news.test(s))add("👀",7);if(RX_PATTERNS.travel.test(s))add("✨",5);if(RX_PATTERNS.food.test(s))add("❤️",5);if(RX_PATTERNS.relationship.test(s))add("❤️",6);if(/[?؟]/.test(s))add("🤔",5);if(/[!！]{2,}/.test(s))add("🔥",4);let candidates=[...score.entries()].sort((a,b)=>b[1]-a[1]);if(!candidates.length||candidates[0][1]<=0)candidates=[["👍",1]];const last=recentReactions.get(String(userId))?.emoji;if(last&&candidates.length>1&&candidates[0][0]===last)candidates=candidates.slice(1);const best=candidates[0]?.[0]||"👍";if(userId)recentReactions.set(String(userId),{emoji:best,expires:Date.now()+REACTION_MEMORY_TTL_MS});return best}
...
async function editMessageRich(chatId,messageId,text){const clean=String(text??"");if(!clean)return{ok:false,description:"Empty message"};const rich=await tg("editMessageText",{chat_id:chatId,message_id:messageId,rich_message:{markdown:clean,is_rtl:detectRtl(clean)}});if(rich?.ok)return rich;if(clean.length<=TG_LIMIT)return tg("editMessageText",{chat_id:chatId,message_id:messageId,text:clean,parse_mode:"MarkdownV2"});return rich}

async function sendRichMessage(chatId,text,replyTo){const clean=String(text??"");if(!clean)return{ok:false,description:"Empty rich message"};const rich=await tg("sendRichMessage",{chat_id:chatId,rich_message:{markdown:clean,is_rtl:detectRtl(clean)},...(replyTo?{reply_parameters:{message_id:replyTo}}:{})});if(rich?.ok)return rich;if(clean.length>TG_LIMIT)return{ok:false,description:"Rich message unavailable for oversized chunk."};return sendMessage(chatId,clean,replyTo,{markdown:true})}

async function sendRichMessageHtml(chatId,html,replyTo){const clean=String(html??"");if(!clean)return{ok:false,description:"Empty rich HTML message"};const result=await tg("sendRichMessage",{chat_id:chatId,rich_message:{html:clean,is_rtl:detectRtl(clean)},...(replyTo?{reply_parameters:{message_id:replyTo}}:{})});return result?.ok?result:{ok:false,description:result?.description||"Rich HTML message failed"}}

async function editMessageRichHtml(chatId,messageId,html){const clean=String(html??"");if(!clean)return{ok:false,description:"Empty rich HTML message"};return tg("editMessageText",{chat_id:chatId,message_id:messageId,rich_message:{html:clean,is_rtl:detectRtl(clean)}})}

async function sendRichMessageDraft(chatId,draftId,text){const clean=String(text??"").slice(0,RICH_LIMIT);return tg("sendRichMessageDraft",{chat_id:chatId,draft_id:draftId,rich_message:{markdown:clean,is_rtl:detectRtl(clean)}},{retries:0})}
async function sendRichMessageDraftHtml(chatId,draftId,html){const clean=String(html??"").slice(0,RICH_LIMIT);return tg("sendRichMessageDraft",{chat_id:chatId,draft_id:draftId,rich_message:{html:clean,is_rtl:detectRtl(clean)}},{retries:0})}
async function typing(chatId){return tg("sendChatAction",{chat_id:chatId,action:"typing"},{retries:0})}

async function reactMessage(chatId,messageId,emoji){if(!RX.includes(emoji)||!chatId||!messageId)return false;for(let attempt=0;attempt<3;attempt++){const result=await tg("setMessageReaction",{chat_id:chatId,message_id:messageId,reaction:[{type:"emoji",emoji}],is_big:false},{retries:0,timeoutMs:10000});if(result?.ok)return true;const description=String(result?.description||"").toLowerCase();if(description.includes("not enough rights")||description.includes("can't react")||description.includes("cannot react")||description.includes("reaction is not allowed")||description.includes("message can't be reacted"))break;if(attempt<2)await sleep(350*(attempt+1))}return false}
...
function imageMagicMatches(buffer,mime){const b=Buffer.from(buffer);if(!b.length)return false;if(mime==="image/jpeg")return b.length>=3&&b[0]===255&&b[1]===216&&b[2]===255;if(mime==="image/png")return b.length>=8&&b.subarray(0,8).equals(Buffer.from([137,80,78,71,13,10,26,10]));if(mime==="image/gif")return b.length>=6&&["GIF87a","GIF89a"].includes(b.subarray(0,6).toString("ascii"));if(mime==="image/webp")return b.length>=12&&b.subarray(0,4).toString("ascii")==="RIFF"&&b.subarray(8,12).toString("ascii")==="WEBP";return false}
function detectImageMime(buffer,path="",httpMime=""){const candidate=normalizeMime(httpMime),extMime=mimeFromExtension(path);if(IMAGE_MIMES.has(candidate)&&imageMagicMatches(buffer,candidate))return candidate;if(IMAGE_MIMES.has(extMime)&&imageMagicMatches(buffer,extMime))return extMime;return""}
function detectFileMime(fileName,declaredMime,httpMime,buffer){const candidates=[normalizeMime(declaredMime),normalizeMime(httpMime),mimeFromExtension(fileName)].filter(Boolean);for(const mime of candidates){if(mime==="application/octet-stream")continue;if(IMAGE_MIMES.has(mime)){if(imageMagicMatches(buffer,mime))return mime;continue}return mime}return""}
async function telegramFile(fileId){const meta=await tg("getFile",{file_id:fileId});if(!meta.ok)throw new Error("Telegram could not resolve the file.");const path=String(meta.result?.file_path||""),fileSize=Number(meta.result?.file_size||0);if(!path)throw new Error("Telegram returned no file path.");if(fileSize&&fileSize>MAX_DOWNLOAD_BYTES)throw new Error(`File is too large to process here (max ${Math.floor(MAX_DOWNLOAD_BYTES/(1024*1024))} MB).`);const response=await fetch(`${TG_API}/file/bot${BOT_TOKEN}/${path}`,{signal:AbortSignal.timeout(REQUEST_TIMEOUT_MS)});if(!response.ok)throw new Error(`File download failed (${response.status}).`);const httpMime=normalizeMime(response.headers.get("content-type")),contentLength=Number(response.headers.get("content-length")||0);if(contentLength>MAX_DOWNLOAD_BYTES)throw new Error(`File is too large to process here (max ${Math.floor(MAX_DOWNLOAD_BYTES/(1024*1024))} MB).`);const arrayBuffer=await response.arrayBuffer();if(arrayBuffer.byteLength>MAX_DOWNLOAD_BYTES)throw new Error(`File is too large to process here (max ${Math.floor(MAX_DOWNLOAD_BYTES/(1024*1024))} MB).`);const buffer=Buffer.from(arrayBuffer);return{buffer,path,httpMime,fileSize:buffer.length,base64:buffer.toString("base64")}}
function decodeTextFile(buffer,fileName,mime=""){const normalized=normalizeMime(mime),ext=extensionOf(fileName),textLike=TEXT_MIMES.has(normalized)||["txt","md","markdown","csv","json","js","mjs","cjs","ts","jsx","tsx","py","java","c","h","cpp","hpp","cs","go","rs","php","rb","sh","bash","html","htm","css","xml","yaml","yml","log"].includes(ext);if(!textLike)return null;return Buffer.from(buffer).toString("utf8").replace(/^\uFEFF/,"").slice(0,MAX_FILE_TEXT_CHARS)}
  ...
  try{
    ...
    if(!isPrivate){
      ...
      if(!placeholder?.ok||!placeholder?.result?.message_id){
        throw new Error("Could not create the Telegram streaming message.");
      }
      ...
      draftChain=draftChain.then(async()=>{
        ...
        if(fallback?.ok){
          draftFallbackMessageId=fallback.result?.message_id||null;
        }
      });
    }
    ...
    /* IMPORTANT:
       No "almost ready" phase.
       No fake final reasoning phase.
       We leave the current rich streamed
       answer alone once generation exists.
    */
    stopStatus();
    ...
    console.log(
      `[complete] model=${sanitizeLog(finalModel)} chars=${full.length} total_ms=${Date.now()-started} searches=${searchState.count}`
    );
    ...
    console.error(
      `[generate:error] model=${sanitizeLog(finalModel||"unknown")} status=${error?.status||"n/a"} message=${sanitizeLog(error?.message||error)}`
    );
...
async function sendRichChunked(chatId,text,replyTo){
  ...
  for(
    ...
    if(!result.ok){
      throw new Error(
        "Telegram could not send the Rich Text response."
      );
    }
...
async function handleUpdate(update){
  ...
  if(!isPrivate){
    const targeted=
      botMentioned(sourceText)||
      triggerUsed(sourceText)||
      repliedToBot(message);
  ...
  if(unsupportedMedia){
    await sendRichMessage(
      chatId,
      "📝 *Type a prompt or send an image.*",
      messageId
    );
    return;
  }
  ...
  /*
    Reaction is sent to the USER'S message,
    not the bot's response.
    It retries briefly because Telegram can
    transiently reject a reaction immediately
...
function userFacingError(error){
  ...
  if(
    ...
    return(
      "❌ The AI provider rejected the request. Please check the bot configuration."
    );
  }
