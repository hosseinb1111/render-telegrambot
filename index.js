...
function thinkingText(label,frame=0){return`${label} ${THINKING_FRAMES[frame%THINKING_FRAMES.length]}`;}

let botId=null,modelCatalog=null,modelCatalogLoadedAt=0,modelCatalogAttemptedAt=0,botInfo=null;

function configWarnings(){const missing=[];if(!BOT_TOKEN)missing.push("BOT_TOKEN");if(!OR_KEY)missing.push("OPENROUTER_API_KEY");if(!WEBHOOK_SECRET)missing.push("WEBHOOK_SECRET");if(!WEBHOOK_PATH_TOKEN)missing.push("WEBHOOK_PATH_TOKEN");if(missing.length)console.error(`Missing required environment variables: ${missing.join(", ")}`);}
configWarnings();

const memory=new Map,prefs=new Map,recentReactions=new Map,inFlightQueues=new Map,seenUpdates=new Map;

function memGet(key){const item=memory.get(key);if(!item)return null;if(item.expires&&Date.now()>item.expires){memory.delete(key);return null;}return item.value;}
function memSet(key,value,ttlSeconds=0){if(memory.has(key))memory.delete(key);memory.set(key,{value,expires:ttlSeconds>0?Date.now()+ttlSeconds*1000:0});trimMap(memory,MAX_MEMORY_ENTRIES);}
function trimMap(map,maxSize){while(map.size>maxSize){const first=map.keys().next().value;if(first===undefined)break;map.delete(first);}}
function cleanupMemory(){const now=Date.now();for(const[key,item]of memory)if(item.expires&&now>item.expires)memory.delete(key);for(const[key,expiresAt]of seenUpdates)if(now>expiresAt)seenUpdates.delete(key);for(const[key,item]of recentReactions)if(now>item.expires)recentReactions.delete(key);}
const memoryCleanupTimer=setInterval(cleanupMemory,300000);memoryCleanupTimer.unref?.();

function historyKey(userId){return`history:${String(userId)}`;}
function getHistory(userId){const raw=memGet(historyKey(userId));if(!raw)return[];try{const parsed=JSON.parse(raw);if(!Array.isArray(parsed))return[];return parsed.filter(item=>item&&(item.role==="user"||item.role==="assistant")&&typeof item.content==="string"&&item.content.length>0);}catch{return[];}}
function saveHistory(userId,prompt,answer){const cleanPrompt=String(prompt||"").slice(0,MAX_USER_PROMPT_CHARS),cleanAnswer=String(answer||"").slice(0,RICH_LIMIT);if(!cleanPrompt||!cleanAnswer)return;const history=getHistory(userId);history.push({role:"user",content:cleanPrompt});history.push({role:"assistant",content:cleanAnswer});memSet(historyKey(userId),JSON.stringify(history.slice(-(HISTORY_PAIRS*2))));enforceHistoryUserLimit();}
function enforceHistoryUserLimit(){let count=0;for(const key of memory.keys())if(key.startsWith("history:"))count++;if(count<=MAX_HISTORY_USERS)return;for(const key of memory.keys()){if(!key.startsWith("history:"))continue;memory.delete(key);if(--count<=MAX_HISTORY_USERS)break;}}
function setPref(userId,key,value){const id=String(userId),current=prefs.get(id)||{};current[key]=String(value).slice(0,200);prefs.set(id,current);trimMap(prefs,MAX_HISTORY_USERS);}
...
function clearAllCache(){memory.clear();prefs.clear();recentReactions.clear();modelCatalog=null;modelCatalogLoadedAt=0;modelCatalogAttemptedAt=0;}
...
function formatUptime(seconds){const d=Math.floor(seconds/86400),h=Math.floor((seconds%86400)/3600),m=Math.floor((seconds%3600)/60),s=seconds%60;return[d?`${d}d`:"",h?`${h}h`:"",m?`${m}m`:"",`${s}s`].filter(Boolean).join(" ");}
function statusText(admin=false){const uptime=Math.floor((Date.now()-stats.started)/1000),lines=["🤖 *Bot Status*","",`Uptime: ${esc(formatUptime(uptime))}`,`Requests: ${stats.requests}`,`Errors: ${stats.errors}`,`Searches: ${stats.searches}`,`Images: ${stats.images}`,`Files: ${stats.files}`,`Avg first token: ${stats.firstTokenMs.length?`${avg(stats.firstTokenMs)} ms`:"—"}`,`Avg total: ${stats.totalMs.length?`${avg(stats.totalMs)} ms`:"—"}`];if(admin)lines.push("",`Telegram errors: ${stats.telegramErrors}`,`OpenRouter errors: ${stats.openRouterErrors}`,`Search failures: ${stats.searchFailures}`,`Memory entries: ${memory.size}`,`Preferences: ${prefs.size}`,`In-flight queues: ${inFlightQueues.size}`);return lines.join("\n");}
...
function chooseReaction(text,image=false,userId=""){const s=String(text||"").trim();if(image&&!s)return"👀";const score=new Map(RX.map(e=>[e,0])),add=(e,p)=>score.set(e,(score.get(e)||0)+p);if(/😭|😢|sad|depressed|غمگین|ناراحت|دلتنگ|ناامید/i.test(s)){add("😢",30);add("😭",10);add("💔",8);}if(/😡|🤬|angry|furious|عصبانی|مزخرف|افتضاح/i.test(s)){add("😡",30);add("👎",7);}if(/😂|🤣|haha+|lol|lmao/i.test(s))add("😂",30);if(/❤️|😍|🥰|love|عشق|دوست دارم/i.test(s))add("❤️",28);if(/🔥|excited|let's go|lets go|بزن بریم/i.test(s))add("🔥",25);if(/🤯|😮|😲|no way|جدی؟|واقعا؟/i.test(s)){add("😮",26);add("🤯",8);}if(RX_PATTERNS.praise.test(s)){add("💯",7);add("👏",5);}if(RX_PATTERNS.help.test(s)){add("🙏",5);add("🤔",4);}if(RX_PATTERNS.code.test(s)){add("💡",6);add("🧠",4);}if(RX_PATTERNS.science.test(s)){add("🧠",7);add("💡",4);}if(RX_PATTERNS.money.test(s)){add("🧐",7);add("💯",3);}if(RX_PATTERNS.news.test(s)){add("🧐",7);add("👀",3);}if(RX_PATTERNS.travel.test(s)){add("✨",6);add("👀",3);}if(RX_PATTERNS.food.test(s)){add("❤️",5);add("✨",2);}if(RX_PATTERNS.relationship.test(s)){add("❤️",7);add("💔",4);}if(/[?؟]/.test(s))add("🤔",5);if(/[!！]{2,}/.test(s))add("🔥",4);if(image)add("👀",8);let candidates=[...score.entries()].sort((a,b)=>b[1]-a[1]);if(!candidates.length||candidates[0][1]<=0)candidates=[["👍",1]];const last=recentReactions.get(String(userId))?.emoji;if(last&&candidates.length>1&&candidates[0][0]===last)candidates=candidates.slice(1);const best=candidates[0]?.[0]||"👍";if(userId)recentReactions.set(String(userId),{emoji:best,expires:Date.now()+REACTION_MEMORY_TTL_MS});return best;}

async function tg(method,body,{retries=1,timeoutMs=REQUEST_TIMEOUT_MS}={}){if(!BOT_TOKEN)return{ok:false,description:"BOT_TOKEN is missing"};for(let attempt=0;attempt<=retries;attempt++){try{const response=await fetch(`${TG_API}/bot${BOT_TOKEN}/${method}`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body??{}),signal:AbortSignal.timeout(timeoutMs)}),raw=await response.text();let data;try{data=raw?JSON.parse(raw):{ok:response.ok};}catch{data={ok:false,description:raw||response.statusText};}if(response.ok&&data?.ok)return data;stats.telegramErrors++;const retryAfter=Number(data?.parameters?.retry_after||0);if(response.status===429&&attempt<retries){await sleep(Math.min(Math.max(retryAfter*1000,250),10000));continue;}return data;}catch(error){stats.telegramErrors++;if(attempt<retries){await sleep(250*(attempt+1));continue;}return{ok:false,description:error instanceof Error?error.message:String(error)};}}return{ok:false,description:"Telegram request failed"};}

async function sendMessage(chatId,text,replyTo,options={}){const clean=String(text??"");if(!clean)return{ok:false,description:"Empty message"};const base={chat_id:chatId,text:clean,...(replyTo?{reply_parameters:{message_id:replyTo}}:{})};if(options.markdown!==false){const richText=await tg("sendMessage",{...base,parse_mode:"MarkdownV2"});if(richText?.ok)return richText;}return tg("sendMessage",base);}
async function sendPlain(chatId,text,replyTo){const clean=String(text??"");if(!clean)return{ok:false,description:"Empty message"};return tg("sendMessage",{chat_id:chatId,text:clean,...(replyTo?{reply_parameters:{message_id:replyTo}}:{})});}
async function editMessagePlain(chatId,messageId,text){const clean=String(text??"");if(!clean)return{ok:false,description:"Empty message"};return tg("editMessageText",{chat_id:chatId,message_id:messageId,text:clean});}
...
async function sendTextMessageDraft(chatId,draftId,text){
  ...
  return tg("sendMessageDraft",{chat_id:chatId,draft_id:draftId,text:clean,parse_mode:"MarkdownV2"},{retries:0});
}
...
async function reactMessage(chatId,messageId,emoji){
  ...
  const result=await tg("setMessageReaction",{chat_id:chatId,message_id:messageId,reaction:[{type:"emoji",emoji}],is_big:false},{retries:0});
  if(!result.ok)console.warn(`Reaction failed: ${sanitizeLog(result.description)}`);
}

async function getMe(){const result=await tg("getMe",{});if(result.ok){botInfo=result.result;botId=result.result.id;}return result;}
...
js:"text/javascript",mjs:"text/javascript",cjs:"text/javascript",ts:"text/typescript",jsx:"text/jsx",tsx:"text/tsx",
py:"text/x-python",java:"text/x-java-source",c:"text/x-c",h:"text/x-c",cpp:"text/x-c++src",hpp:"text/x-c++src",
cs:"text/plain",go:"text/plain",rs:"text/plain",php:"text/plain",rb:"text/plain",sh:"text/x-shellscript",bash:"text/x-shellscript",
html:"text/html",htm:"text/html",css:"text/css",xml:"application/xml",yaml:"application/yaml",yml:"application/yaml",
log:"text/plain",rtf:"application/rtf",pdf:"application/pdf",doc:"application/msword",
...
ods:"application/vnd.oasis.opendocument.spreadsheet",odp:"application/vnd.oasis.opendocument.presentation",zip:"application/zip"
};
...
function imageMagicMatches(buffer,mime){
  const b=Buffer.from(buffer);if(!b.length)return false;
...
function decodeTextFile(buffer,fileName,mime=""){
  const normalized=normalizeMime(mime),ext=extensionOf(fileName),textLike=TEXT_MIMES.has(normalized)||["txt","md","markdown","csv","json","js","mjs","cjs","ts","jsx","tsx","py","java","c","h","cpp","hpp","cs","go","rs","php","rb","sh","bash","html","htm","css","xml","yaml","yml","log"].includes(ext);
...
async function searchWeb(query){
  ...
  let data;try{data=await response.json();}catch{stats.searchFailures++;throw new Error("Search service returned invalid JSON.");}
  ...
  return results.slice(0,5).map((item,index)=>{const name=String(item?.name||"Untitled").slice(0,180),url=String(item?.url||"").slice(0,500),summary=String(item?.summary||item?.snippet||"").slice(0,700);return[`[${index+1}] ${name}`,`URL: ${url}`,summary].join("\n");}).join("\n\n");
}
...
async function buildOpenRouterBody(messages,model,mode,tools){
  const body={model,messages,stream:true};let info=null;
  if(mode==="deep"||tools?.length)info=await getModelInfo(model);
  ...
  if(mode==="deep"&&supported.has("reasoning")){
    const efforts=info?.reasoning?.supported_efforts;
    if(Array.isArray(efforts)&&efforts.length){
      const desired=["high","max","xhigh","medium"].find(e=>efforts.includes(e));
      body.reasoning={effort:desired||efforts[0],exclude:true};
    }else body.reasoning={effort:"high",exclude:true};
  }
  return body;
}
...
async function orRequest(messages,model,mode,tools=null){
  ...
  const body=await buildOpenRouterBody(messages,model,mode,tools);
  let response;
  try{
  ...
  if(!response.ok){
    stats.openRouterErrors++;
    const raw=await response.text().catch(()=>""),suffix=(()=>{try{const parsed=raw?JSON.parse(raw):null;const detail=parsed?.error?.message||parsed?.message||"";return detail?`: ${sanitizeLog(detail).slice(0,300)}`:"";}catch{return raw?`: ${sanitizeLog(raw).slice(0,300)}`:"";}})();
...
function cleanSystem(userId,mode){
  const extra=[];
  if(mode==="fast")extra.push("Be concise and prioritize speed. Do not search the web unless the system permits it.");
  if(mode==="deep")extra.push("Be thorough. Verify time-sensitive claims with web search when appropriate.");
  if(LANGSEARCH_KEY&&mode!=="fast")extra.push("Use the web_search tool whenever information is current, recent, live, unstable, niche, or externally verifiable. Prefer searching over saying you do not know when external verification could answer the question. Do not search unnecessarily.");
  ...
  const prefStyle=getPref(userId,"style"),prefLang=getPref(userId,"language");
  if(prefStyle)extra.push(`Preferred style: ${prefStyle}.`);
  if(prefLang)extra.push(`Preferred language: ${prefLang}.`);
  return SYSTEM_PROMPT+(extra.length?`\n\n${extra.join("\n")}`:"");
}

async function buildMessages({userId,prompt,image,file,fileText,mode}){
  const messages=[{role:"system",content:cleanSystem(userId,mode)}];
  if(!image&&!file)messages.push(...getHistory(userId).slice(-(HISTORY_PAIRS*2)));
  if(image){
...
function processSseLine(rawLine,onPiece,toolCalls){
  const line=rawLine.trim();if(!line.startsWith("data:"))return;
...
async function performToolCalls(toolCalls,searchState){
  const assistantToolCalls=toolCalls.map(call=>({id:call.id,type:"function",function:{name:"web_search",arguments:call.function?.arguments||"{}"}}));
  const toolMessages=[];
  for(const call of toolCalls){
    ...
    if(!query)result="The search request was invalid because no query was supplied.";
      ...
      searchState.count++;
      try{result=await searchWeb(query);}
      catch(error){result=`Search failed gracefully: ${error instanceof Error?error.message:"unknown error"}`;}
    }
...
async function generate({chatId,userId,prompt,image,file,fileText,mode,replyTo,isPrivate}){
  ...
  let statusFrame=0,statusTimer=null,statusLabel="🧠 *Thinking…*";
  ...
  const setStatus=label=>{
    ...
    statusFrame=0;
    const tick=()=>{
      const text=thinkingText(statusLabel,statusFrame++);
      if(isPrivate){
        draftChain=draftChain.then(async()=>{
          if(useRichDraft)await sendRichMessageDraft(chatId,draftIdValue,text).catch(()=>{});
          else if(draftFallbackMessageId)await editMessageRich(chatId,draftFallbackMessageId,text).catch(()=>{});
        });
      }else if(streamMessageId){
        groupEditChain=groupEditChain.then(async()=>{await editMessageRich(chatId,streamMessageId,text).catch(()=>{});});
      }
    };
    tick();
    statusTimer=setInterval(tick,650);
    statusTimer.unref?.();
  };
  ...
  try{
    ...
    if(!isPrivate){
      const placeholder=await sendRichMessage(chatId,thinkingText("🧠 *Thinking…*"),replyTo);
      ...
      draftChain=draftChain.then(async()=>{
        const result=await sendRichMessageDraft(chatId,draftIdValue,thinkingText("🧠 *Thinking…*"));
        if(result.ok)return;
        useRichDraft=false;
        const fallback=await sendRichMessage(chatId,thinkingText("🧠 *Thinking…*"),replyTo);
    ...
    for(let round=0;round<MAX_TOOL_ROUNDS;round++){
      let roundText="";
      const toolCalls=[];
      const allowTools=Boolean(LANGSEARCH_KEY&&mode!=="fast"&&searchState.count<MAX_SEARCHES);

      const response=await orRequest(messages,finalModel,mode,allowTools?SEARCH_TOOL:null);
        ...
        piece=>{
          ...
          if(isPrivate){
            ...
            groupEditChain=groupEditChain.then(async()=>{
              await editMessageRich(chatId,streamMessageId,preview).catch(()=>{});
            }).catch(()=>{});
          }
        },
      ...
      if(validTools.some(call=>call?.function?.name==="web_search")){
        stopStatus();
      ...
      stopStatus();
      setStatus("🧠 *Reasoning from search results…*");
    }

    stopStatus();
    setStatus("✍️ *Preparing the final answer…*");
    ...
    if(isPrivate){
      if(draftFallbackMessageId){
        ...
        if(finalParts[0]){
          await editMessageRich(chatId,draftFallbackMessageId,finalParts[0]);
        }
    ...
    saveHistory(userId,prompt,full);

    const reaction=chooseReaction(prompt,Boolean(image),userId);
    const reactionMessageId=streamMessageId||draftFallbackMessageId;
    if(reactionMessageId)reactMessage(chatId,reactionMessageId,reaction).catch(()=>{});
    ...
    try{
      if(!full.trim()){
        if(streamMessageId){
          await editMessageRich(chatId,streamMessageId,publicMessage);
        }else if(draftFallbackMessageId){
          await editMessageRich(chatId,draftFallbackMessageId,publicMessage);
        }else{
          await sendRichMessage(chatId,publicMessage,replyTo);
        }
      }else{
...
async function command(chatId,userId,text,messageId){
  const parsed=parseCommand(text);if(!parsed)return false;
  switch(parsed.command){
    case"/start":
      await sendRichMessage(chatId,["🤖 *Welcome.*","","Send me a message to chat. In groups, mention me, reply to me, or use the trigger command.","","Use /help to see everything I support."].join("\n"),messageId);return true;
    case"/help":await sendRichMessage(chatId,helpText(),messageId);return true;
    case"/fast":setPref(userId,"mode","fast");await sendRichMessage(chatId,"⚡ *Fast mode enabled.*",messageId);return true;
    case"/normal":setPref(userId,"mode","normal");await sendRichMessage(chatId,"🙂 *Normal mode enabled.*",messageId);return true;
    case"/deep":setPref(userId,"mode","deep");await sendRichMessage(chatId,"🧠 *Deep mode enabled.*",messageId);return true;
    case"/style":
      if(!parsed.arg)await sendRichMessage(chatId,"Usage: `/style concise`",messageId);
      else{setPref(userId,"style",parsed.arg);await sendRichMessage(chatId,`✍️ Style set to: *${esc(parsed.arg)}*`,messageId);}
      return true;
    case"/language":
      if(!parsed.arg)await sendRichMessage(chatId,"Usage: `/language English`",messageId);
      else{setPref(userId,"language",parsed.arg);await sendRichMessage(chatId,`🌐 Language set to: *${esc(parsed.arg)}*`,messageId);}
      return true;
    case"/clear":
    case"/clearmemory":
      clearUserMemory(userId);await sendRichMessage(chatId,"🧹 *Conversation memory cleared.*",messageId);return true;
    case"/status":await sendRichMessage(chatId,statusText(false),messageId);return true;
    case"/stats":await sendRichMessage(chatId,statusText(false),messageId);return true;
    case"/models":
      await sendRichMessage(chatId,["🤖 *Active models*","",`Normal: \`${escCode(TEXT_MODEL)}\``,`Fast: \`${escCode(FAST_MODEL)}\``,`Deep: \`${escCode(DEEP_MODEL)}\``,`Vision: \`${escCode(VISION_MODEL)}\``,`Files: \`${escCode(FILE_MODEL)}\``].join("\n"),messageId);return true;
    default:return false;
  }
}

function helpText(){
  return["🤖 *How to use the bot*","","💬 *Chat*","• In a private chat, send a normal message.","• In a group, mention me, reply to one of my messages, or use the trigger command.",`• Trigger command: \`${esc(TRIGGER||"!ai")}\` your message`,"","⚡ *Modes*","• `/fast` — prioritize speed and concise answers.","• `/normal` — balanced default behavior.","• `/deep` — more thorough reasoning and web research when useful.","","🧠 *Memory*",`• The bot keeps the most recent ${HISTORY_PAIRS} conversation pair${HISTORY_PAIRS===1?"":"s"} in memory per user.`,"• `/clear` or `/clearmemory` clears your conversation history.","• Memory is local to the running service and resets when it restarts.","","🌐 *Web search*","• The bot can automatically search when information needs current, recent, live, niche, or external verification.","• Search is not used for every message, and fast mode does not use the search tool.","","🖼 *Images*","• Send an image with an optional question or instruction.","• Images are automatically routed to the configured vision model.","• Supported image formats: JPEG, PNG, WEBP, and GIF.","","📄 *Files*","• You can attach common documents and text/code files.","• The bot safely forwards supported files to the configured file model; text-like files can also be processed as text.","• Processing is subject to Telegram/OpenRouter file-size and model limits.","","⚙️ *Preferences*","• `/style concise` — choose a response style.","• `/language English` — choose a preferred response language.","","📊 *Info*","• `/status` — current runtime statistics.","• `/stats` — same user-safe statistics view.","• `/models` — active text, fast, deep, vision, and file models.","• `/help` — show this guide again."].join("\n");
}

const ADMIN_IDS=new Set(String(process.env.ADMIN_IDS||"").split(",").map(value=>value.trim()).filter(Boolean));

async function adminCommand(chatId,userId,text,messageId){
  if(!ADMIN_IDS.has(String(userId)))return false;
  const parsed=parseCommand(text);if(!parsed)return false;
  switch(parsed.command){
    case"/admin":await sendRichMessage(chatId,"🔐 *Admin access active.*",messageId);return true;
    case"/adminstats":await sendRichMessage(chatId,statusText(true),messageId);return true;
    case"/clearall":clearAllCache();await sendRichMessage(chatId,"🧹 *All local cache and memory cleared.*",messageId);return true;
    case"/reloadmodels":await fetchModelCatalog(true);await sendRichMessage(chatId,"🔄 *Model catalog refreshed.*",messageId);return true;
    default:return false;
  }
}

async function finalizeGroup(chatId,messageId,text){
  const parts=splitText(text,RICH_LIMIT);if(!parts.length)return;
  let first=await editMessageRich(chatId,messageId,parts[0]);
  if(!first.ok&&parts[0].length<=TG_LIMIT){
    first=await tg("editMessageText",{chat_id:chatId,message_id:messageId,text:parts[0],parse_mode:"MarkdownV2"});
  }
  for(let i=1;i<parts.length;i++){await sendRichMessage(chatId,parts[i]);if(i<parts.length-1)await sleep(40);}
}

async function sendRichChunked(chatId,text,replyTo){
  const richParts=splitText(text,RICH_LIMIT);let first=true;
  for(const richPart of richParts){
    const result=await sendRichMessage(chatId,richPart,first?replyTo:undefined);
    if(result.ok){first=false;await sleep(40);continue;}
    if(richPart.length<=TG_LIMIT){
      const fallback=await tg("sendMessage",{chat_id:chatId,text:richPart,parse_mode:"MarkdownV2",...(first&&replyTo?{reply_parameters:{message_id:replyTo}}:{})});
      if(!fallback.ok)throw new Error("Telegram could not send the final response.");
      first=false;
      await sleep(40);
    }else throw new Error("Telegram could not send the final response.");
  }
}

function splitText(text,limit){
  const input=String(text||"");if(!input)return[];if(input.length<=limit)return[input];
  const result=[];let start=0;
  while(start<input.length){
    const maxEnd=Math.min(start+limit,input.length);
    if(maxEnd>=input.length){result.push(input.slice(start).trim());break;}
    const window=input.slice(start,maxEnd);
    const newline=window.lastIndexOf("\n\n"),newlineSingle=window.lastIndexOf("\n");
    ...
    const part=input.slice(start,cut).trim();if(part)result.push(part);start=cut;
  }
  return result;
}

function parseCommand(text){
  const raw=String(text||"").trim();if(!raw.startsWith("/"))return null;
  const[first,...rest]=raw.split(/\s+/),[commandName]=first.split("@");
...
function detectRtl(text){const value=String(text||""),rtl=(value.match(/[\u0590-\u08ff]/g)||[]).length,ltr=(value.match(/[A-Za-z]/g)||[]).length;return rtl>10&&rtl>=ltr*.25;}
function userFacingError(error){const status=Number(error?.status||0);if(status===401||status===403)return"❌ The AI provider rejected the request. Please check the bot configuration.";if(status===429)return"❌ The AI service is temporarily rate-limited. Please try again in a moment.";if(status>=500)return"❌ The AI service is temporarily unavailable. Please try again shortly.";return"❌ I could not complete that request right now. Please try again.";}

function safeHeaderEqual(a,b){
  const x=Buffer.from(String(a||"")),y=Buffer.from(String(b||""));if(x.length!==y.length)return false;
  let diff=0;for(let i=0;i<x.length;i++)diff|=x[i]^y[i];return diff===0;
}

function messageUserId(message){return message?.from?.id||message?.sender_chat?.id||message?.chat?.id||0;}
function messageChatId(message){return message?.chat?.id||0;}
function extractMessageText(message){return message?.text||message?.caption||"";}
function detectMode(userId){return getPref(userId,"mode")||"normal";}

function isBotMentioned(text){
  if(!BOT_USERNAME)return false;
  return new RegExp(`(^|\\s)@${escapeRegExp(BOT_USERNAME)}\\b`,"i").test(String(text||""));
}

function shouldHandleMessage(message){
  if(!message)return false;
  const text=extractMessageText(message);
  if(message.chat?.type==="private")return Boolean(text||message.photo||message.document);
  if(isBotMentioned(text))return true;
  if(message.reply_to_message&&botId&&message.reply_to_message.from?.id===botId)return true;
  if(TRIGGER&&text.trim().startsWith(TRIGGER))return true;
  return false;
}

function stripTrigger(text){
  let value=String(text||"").trim();
  if(BOT_USERNAME)value=value.replace(new RegExp(`^@${escapeRegExp(BOT_USERNAME)}\\b\\s*`,"i"),"");
  if(TRIGGER)value=value.replace(new RegExp(`^${escapeRegExp(TRIGGER)}\\s*`),"");
  if(BOT_USERNAME)value=value.replace(new RegExp(`@${escapeRegExp(BOT_USERNAME)}\\b`,"ig"),"");
  return value.trim();
}

async function extractAttachment(message){
  if(message?.photo?.length){
    const photo=message.photo[message.photo.length-1],file=await telegramFile(photo.file_id),mime=detectImageMime(file.buffer,file.path,file.httpMime);
    if(!mime)throw new Error("Unsupported or invalid image.");
    stats.images++;
    return{image:{mime,base64:file.base64},file:null,fileText:null};
  }

  if(message?.document){
    const doc=message.document,file=await telegramFile(doc.file_id),mime=detectFileMime(doc.file_name||"file",doc.mime_type,file.httpMime,file.buffer);
    if(!mime)throw new Error("Unsupported file type.");
    const fileText=decodeTextFile(file.buffer,doc.file_name||"file",mime);
    stats.files++;
    const part=fileText?null:filePartFromDownload(file,doc.file_name||"file",doc.mime_type);
    return{image:null,file:{name:doc.file_name||"file",part},fileText};
  }

  return{image:null,file:null,fileText:null};
}

function queueForUser(userId,fn){
  const key=String(userId),previous=inFlightQueues.get(key)||Promise.resolve();
  const current=previous.catch(()=>{}).then(fn);
  inFlightQueues.set(key,current.finally(()=>{if(inFlightQueues.get(key)===current)inFlightQueues.delete(key);}));
  return current;
}

async function handleMessage(message){
  if(!message)return;
  const chatId=messageChatId(message),userId=messageUserId(message),messageId=message.message_id;
  if(!chatId||!userId)return;
  if(!shouldHandleMessage(message))return;

  const text=extractMessageText(message);

  if(text.startsWith("/")&&await adminCommand(chatId,userId,text,messageId))return;
  if(text.startsWith("/")&&await command(chatId,userId,text,messageId))return;

  const prompt=stripTrigger(text);

  if(!prompt&&!message.photo&&!message.document){
    await sendRichMessage(chatId,"💬 Send me a question or instruction.",messageId);
    return;
  }

  let attachments;
  try{attachments=await extractAttachment(message);}
  catch(error){
    await sendRichMessage(chatId,`❌ *Could not process the attachment.*\n\n${esc(error instanceof Error?error.message:"Unknown error")}`,messageId);
    return;
  }

  const mode=detectMode(userId),isPrivate=message.chat?.type==="private";

  await queueForUser(userId,()=>generate({
    chatId,userId,prompt,image:attachments.image,file:attachments.file,fileText:attachments.fileText,
    mode,replyTo:messageId,isPrivate
  }));
}

async function handleUpdate(update){
  if(!update)return;
  const key=String(update?.update_id||`${Date.now()}:${Math.random()}`);
...
function drainJobs(){
  while(activeJobs<MAX_GLOBAL_CONCURRENCY&&pendingJobs.length){
    const job=pendingJobs.shift();activeJobs++;
...
if(RAW_WEBHOOK_PATH_TOKEN&&!/^[A-Za-z0-9._~-]{8,256}$/.test(RAW_WEBHOOK_PATH_TOKEN)){
  console.error("WEBHOOK_PATH_TOKEN contains unsafe URL characters. Use a URL-safe token.");
}
...
app.post(WEBHOOK_PATH,(req,res)=>{
  ...
  processUpdate(req.body).catch(error=>{
    console.error(`[processUpdate:error] ${sanitizeLog(error?.message||error)}`);
  });
});
...
app.post("/guest",(req,res)=>{
  ...
  if(!safeHeaderEqual(req.get("X-Guest-Secret"),GUEST_SECRET)){
    return res.status(401).type("text/plain").send("Unauthorized");
  }

  const body=req.body||{},chatId=body.chat_id??body.chatId;
  const prompt=String(body.prompt??body.text??"").trim().slice(0,MAX_USER_PROMPT_CHARS);

  if(!chatId||!prompt){
    return res.status(400).json({ok:false,error:"chat_id and prompt are required"});
  }
  ...
  enqueueUser(
    `guest:${chatId}`,
    ()=>withGlobalConcurrency(()=>generate({
      chatId,
      userId:`guest:${chatId}`,
      prompt,
      image:null,
...
const server=app.listen(PORT,async()=>{
  ...
  try{
    ...
    if(me.ok){
      console.log(`Bot connected${me.result?.username?` as @${sanitizeLog(me.result.username)}`:""}.`);
    }else{
      console.error(`Telegram getMe failed: ${sanitizeLog(me.description)}`);
    }
  }catch(error){
...
server.on("error",error=>{
  console.error(`HTTP server error: ${sanitizeLog(error?.message||error)}`);
});

process.on("unhandledRejection",reason=>{
  console.error(`[unhandledRejection] ${sanitizeLog(reason?.message||reason)}`);
});

process.on("uncaughtException",error=>{
  console.error(`[uncaughtException] ${sanitizeLog(error?.message||error)}`);
});

function enqueueUser(userId,task){
  return queueForUser(userId,task);
}
